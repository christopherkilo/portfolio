    ...
    <div className="space-y-8">
      ...
      <section className="rounded-3xl border border-white/8 bg-white/[0.03] p-5 sm:p-6">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          ...
          <label className="text-sm text-muted">Applications commonly open<input type="range" min="2" max="24" value={appCount} onChange={(event) => setAppCount(Number(event.target.value))} className="mt-4 w-full accent-primary" /><span className="mt-1 block text-text">{appCount} applications</span></label>
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return <label className="flex items-center justify-between rounded-xl border border-white/8 p-3 text-sm text-muted"><span>{label}</span><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="size-4 accent-primary" /></label>;
}
