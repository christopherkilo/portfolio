  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-3xl border border-white/8 bg-white/[0.03] p-5 sm:p-6">
        ...
        <dl className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[["Local IPv4", network.ipv4], ["Gateway", network.gateway], ["DNS provider", network.dnsProvider], ["Public IP", network.publicIp], ["Network profile", network.profile]].map(([label, value]) => <div key={label} className="rounded-xl bg-black/20 p-3"><dt className="text-xs uppercase tracking-wider text-muted">{label}</dt><dd className="mt-1 font-mono text-sm text-text">{value}</dd></div>)}</dl>
      </section>
